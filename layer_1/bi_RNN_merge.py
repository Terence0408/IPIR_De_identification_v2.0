...
def what_d(runtimes = 1, renew =True, maxlen=100):
    ...
    f.write("22 times bi_RNN_merge" + str(file_id) + " cosine similarity: "+str(sum(cos)/len(cos))+"\n")
    f.close()
    ...
    for i in range(0, len(vocab)):
        ...
        if len(misspelling)>4:
            ...
            cos.append(1 - spatial.distance.cosine(map_LSTM, map_GloVe))
    f = open(path+"layer_1/cosine.txt", 'a')
    f.write("22 times bi_RNN_merge" + str(file_id) + " misspelling cosine similarity : "+str(sum(cos)/len(cos))+", len: "+str(len(cos))+"\n")
    f.close()

what_d(runtimes =  18, renew =False,  maxlen = 18)

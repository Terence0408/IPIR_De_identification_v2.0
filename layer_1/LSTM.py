...
def what_d(runtimes = 1, renew =True, maxlen=100, file_id=2):
    ...
    if renew == True:
        ...
        model.save(path + "layer_1/LSTM" + str(file_id) + ".pk")


    # Not first time: build the model: a bidirectional LSTM

    print('Load model...')
    model = load_model(path+"layer_1/LSTM" + str(file_id) + ".pk")
    ...
    f.write("20 times LSTM" + str(file_id) + " cosine similarity: "+str(sum(cos)/len(cos))+"\n")
    f.close()
    ...
    for i in range(0, len(vocab)):
        ...
        if len(misspelling)>4:
            ...
            cos.append(1 - spatial.distance.cosine(map_LSTM, map_GloVe))
    f = open(path+"layer_1/cosine.txt", 'a')
    f.write("20 times LSTM" + str(file_id) + " misspelling cosine similarity : "+str(sum(cos)/len(cos))+", len: "+str(len(cos))+"\n")
    f.close()

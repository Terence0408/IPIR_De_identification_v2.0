...
def what_d(runtimes = 1, renew =True, maxlen=100, file_id=2):
    ...
    if renew == True:
        print('Build model...')
        left = Sequential()
        left.add(GRU(char_Y_01/2, input_shape=(char_X_010, char_X_001), activation='tanh',
                           inner_activation='sigmoid', dropout_W=0.5, dropout_U=0.5))
                           #dropout_W=0.5, dropout_U=0.5))
        right = Sequential()
        right.add(GRU(char_Y_01/2, input_shape=(char_X_010, char_X_001), activation='tanh',
        ...
        model.save(path + "layer_2/bi_GRU_concat__dense" + str(file_id) + ".pk")


    # Not first time: build the model: a bidirectional LSTM

    print('Load model...')
    model = load_model(path+"layer_2/bi_GRU_concat__dense" + str(file_id) + ".pk")
    ...
    f.write("20 times bi_GRU_concat__dense" + str(file_id) + " cosine similarity: "+str(sum(cos)/len(cos))+"\n")
    f.close()
    ...
    for i in range(0, len(vocab)):
        ...
        if len(misspelling)>4:
            ...
            cos.append(1 - spatial.distance.cosine(map_LSTM, map_GloVe))
    f = open(path+"layer_2/cosine.txt", 'a')
    f.write("20 times bi_GRU_concat__dense" + str(file_id) + " misspelling cosine similarity : "+str(sum(cos)/len(cos))+", len: "+str(len(cos))+"\n")
    f.close()
